...
def load_final_regret_results(filepath):
    ...
    try:
        ...
        print(
            f"{'Algorithm':<20} {'Mean Regret':<12} {'Std Dev':<10} {'95% CI Margin':<12}"
        )
        print("-" * 60)
        ...
        for key in keys:
            ...
            print(
                f"{display_name:<20} {mean_regret:<12.2f} {std_regret:<10.2f} {ci_margin:<12.2f}"
            )
...
def main():
    ...
    if final_regret_data is not None:
        ...
        regrets_with_ci = [
            (key, values[0], values[2]) for key, values in final_regret_data.items()
        ]
        ...
        for i, (key1, mean1, ci1) in enumerate(regrets_with_ci):
            for j, (key2, mean2, ci2) in enumerate(regrets_with_ci[i + 1 :], i + 1):
                ...
                significance = (
                    "Not significant" if overlap else "Significantly different"
                )
