...
def create_slide_figure():
    ...
    print(f"Creating slide figure from ALL {len(xyzwst_data)} XYZWST topological orderings...")
    ...
    ax1.text(0.02, 0.98, stats_text, transform=ax1.transAxes, fontsize=10,
             verticalalignment='top', bbox=dict(boxstyle='round,pad=0.4', facecolor='lightgreen', alpha=0.8))
    
    # Add legend in a better position
    legend_elements = [plt.Line2D([0], [0], color='green', linestyle='--', label=f'Min: {times.min():.3f} ms'),
                      plt.Line2D([0], [0], color='orange', linestyle='-', label=f'Mean: {times.mean():.3f} ms'),
                      plt.Line2D([0], [0], color='red', linestyle='--', label=f'Max: {times.max():.3f} ms')]
    ...
    bars = ax2.bar(metrics, values, color=['lightcoral', 'lightblue', 'lightgreen'], 
    ...
    ax2.text(0.5, 0.75, 'Same steps for\nall 20 orderings', transform=ax2.transAxes, 
    ...
    plt.savefig('pomis_slide_figure.png', dpi=300, bbox_inches='tight', facecolor='white')
    ...
    print(f"- Algorithm steps: Constant across all orderings")
    print(f"- Result correctness: ✓ Identical POMIS sets")
    print(f"- Complete coverage: ✓ Every valid topological order tested")
