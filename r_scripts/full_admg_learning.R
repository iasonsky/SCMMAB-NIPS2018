...
#   Function that saves a PAG plot to file
save_pag_plot <- function(pag, filename, output_dir = "plots"){
    ...
    png(full_path, width = 800, height = 600, res = 100)
    plot(pag)
    dev.off()
    
    cat(paste("Saved PAG plot:", full_path, "\n"))
}

#   Function that saves ADMG matrices as text files
save_admg_matrices <- function(admg_list, output_dir = "plots", prefix = "admg"){
    # Create output directory if it doesn't exist
    if(!dir.exists(output_dir)) {
        dir.create(output_dir, recursive = TRUE)
    }
    
    cat(paste("Saving", length(admg_list), "ADMG matrices to", output_dir, "\n"))
    
    for(i in 1:length(admg_list)){
        filename <- file.path(output_dir, paste0(prefix, "_", i, "_matrix.txt"))
        
        # Write matrix to text file
        write.table(admg_list[[i]], filename, sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)
        cat(paste("Saved matrix:", filename, "\n"))
    }
}
