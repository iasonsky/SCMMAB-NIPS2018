...
def pag_to_admg(pag_adj: np.ndarray, var_names: List[str], edges: List[Tuple[str, str]]) -> List[np.ndarray]:
    """
...
def _save_pag_admg_results(
    ...
    # Save FCI PAG matrix
    np.savetxt(output_dir / "pag_fci_format.txt", pag_adj_fci, fmt='%d', 
               header=f"FCI PAG format\nVariables: {var_names}")
    
    # Save R PAG matrix
    np.savetxt(output_dir / "pag_r_format.txt", pag_adj_r, fmt='%d',
               header=f"R PAG format\nVariables: {var_names}")
    
    # Save ADMG matrices
    for i, admg in enumerate(admg_matrices):
        np.savetxt(output_dir / f"admg_{i+1}.txt", admg, fmt='%d',
                   header=f"ADMG {i+1}\nVariables: {var_names}")
    
    # Save summary
    with open(output_dir / "summary.txt", "w") as f:
        f.write(f"PAG to ADMG Conversion Results\n")
    ...
    print(f"   Files saved: pag_fci_format.txt, pag_r_format.txt, admg_*.txt, summary.txt")
