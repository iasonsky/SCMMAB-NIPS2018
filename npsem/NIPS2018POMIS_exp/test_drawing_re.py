...
from npsem.NIPS2018POMIS_exp.test_bandit_strategies import (
    load_result,
...
def naked_MAB_optimal_probability_plot(
    axes: Axes,
    arm_freqs,
    cut_time,
    legend=False,
...
def aggregate_plot():
    """Prepare data"""
    info__ = {
        "Markovian": (absolute_path + "bandit_results/mark_0", 1000),
        "IV": (absolute_path + "bandit_results/iv_0", 1000),
        "XYZWST": (absolute_path + "bandit_results/xyzwst_0", 10000),
    }
    info = {k: dict(zip(["directory", "cut_time"], v)) for k, v in info__.items()}
    results = {
        task_name: dict(zip(["CR", "OAP"], data_prep(task_info["directory"])))
        for task_name, task_info in info.items()
    }
    plot_funcs = {
        "CR": naked_MAB_regret_plot,
        "OAP": naked_MAB_optimal_probability_plot,
    }
    ...
    for row_id, plot_type in enumerate(["CR", "OAP"]):
        for col_id, task_name in enumerate(["Markovian", "IV", "XYZWST"]):
            ...
            plot_funcs[plot_type](
                ...
                hide_yticklabels=False,
                adjust_ymax={"Markovian": 1, "IV": 0.35, "XYZWST": 0.55}[task_name]
                if plot_type == "CR"
                else 1,
            )
