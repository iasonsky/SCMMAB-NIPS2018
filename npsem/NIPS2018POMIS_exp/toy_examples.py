"""Visualise toy bandit examples using previously computed results.

This script loads results generated by ``test_bandit_strategies.py`` and
produces publication-quality plots for selected horizons.  Run it after the
experiments have finished to generate figures in ``toy_figs``.

Usage
-----
```
python -m npsem.NIPS2018POMIS_exp.toy_examples
```
"""
...
def MAB_optimal_probability_plot2(
    fname,
    arm_freqs,
    colors=None,
    title=None,
    ...
    for i, (name, arm_freq) in enumerate(arm_freqs.items()):
        ...
        plt.plot(
            ...
            lw=1,
            label=name_change[name],
            color=colors[i],
            linestyle="-" if "once" in name else "--",
        )
    if not no_legend:
        plt.legend(loc=4)
    plt.xlabel("Trials")
...
def main_viz2(directory, results, mu, horizons):
    ...
    for cut in horizons:
        MAB_optimal_probability_plot2(
            directory + f"/toy_prob_{str(cut).zfill(5)}.pdf",
            arm_optimality_results,
            colors,
            cut_time=cut,
            fig_kwargs={"figsize": (3.5, 2)},
            name_filter=lambda x: "(TS)" in x and "MIS" not in x,
            name_change={
                "Brute-force (TS)": "All Subsets",
                "All-at-once (TS)": "All-at-once",
            },
        )
        MAB_regret_plot2(
            directory + f"/toy_regret_{str(cut).zfill(5)}.pdf",
            regret_results,
            colors,
            fig_kwargs={"figsize": (3.5, 2)},
            cut_time=cut,
...
def main():
    directory = "bandit_results/iv_0"
    _p_u, _mu, _results = load_result(directory)
    fig_directory = "toy_figs"
