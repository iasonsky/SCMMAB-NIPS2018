...
from npsem.pomis_instrumentation import InstrumentedPOMISComputer, generate_topological_orders_strategies
...
def run_single_experiment(graph_name: str, G: CausalDiagram, Y: str, 
                         output_dir: Path, num_random_orders: int = 10) -> List[Dict[str, Any]]:
    ...
    for strategy_name, order in strategies.items():
        ...
        try:
            ...
            result_matches = (reference_result is None or 
                            frozenset(result_set) == frozenset(reference_result))
            ...
            if not result_matches:
                print(f"    WARNING: Strategy {strategy_name} produced different POMIS set!")
...
def create_plots(results: List[Dict[str, Any]], output_dir: Path):
    ...
    for graph in df['graph_name'].unique():
        graph_data = df[df['graph_name'] == graph]
        ax3.scatter(graph_data['total_subpomis_calls'], 
                   graph_data['total_ib_evaluations'], 
                   alpha=0.7, label=graph)
    ...
    for graph in df['graph_name'].unique():
        graph_data = df[df['graph_name'] == graph]
        for metric in ['total_subpomis_calls', 'total_ib_evaluations', 
                      'total_pruned_branches', 'computation_time', 'max_recursion_depth']:
            stats = {
                'graph': graph,
                'metric': metric,
...
def verify_result_consistency(results: List[Dict[str, Any]]) -> bool:
    ...
    for graph_name in set(r['graph_name'] for r in results):
        ...
        if mismatches:
            print(f"  ❌ Graph '{graph_name}': {len(mismatches)}/{len(graph_results)} orders produced different results!")
            consistent = False
        else:
            print(f"  ✅ Graph '{graph_name}': All {len(graph_results)} orders produced identical POMIS sets")
    
    return consistent


def main():
    parser = argparse.ArgumentParser(description="Study effect of topological ordering on POMIS computation")
    parser.add_argument("--output-dir", "-o", type=Path, default="pomis_topological_results",
                       help="Output directory for results")
    parser.add_argument("--num-random", "-n", type=int, default=20,
                       help="Number of random topological orders to test per graph")
    parser.add_argument("--seed", "-s", type=int, default=42,
                       help="Random seed for reproducibility")
    parser.add_argument("--graphs", nargs="*", 
                       help="Specific graphs to test (default: all available)")
    ...
    print(f"POMIS Topological Ordering Experiment")
    ...
    for graph_name, (G, Y) in graphs.items():
        try:
            results = run_single_experiment(graph_name, G, Y, args.output_dir, args.num_random)
