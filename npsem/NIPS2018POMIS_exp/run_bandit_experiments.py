...
from npsem.NIPS2018POMIS_exp.scm_registry import registry


def run_single_experiment(
    scm_name: str, quick: bool = False, show_plots: bool = True
) -> bool:
    ...
    try:
        ...
        experiment = BanditExperiment(
            scm_factory=scm_factory, scm_name=scm_info["display_name"], **config
        )

        experiment.run_full_experiment(
            save_dir="experiment_results", show_plots=show_plots, y_lim=y_lim
        )
        return True
...
def run_all_experiments(
    scm_names: List[str], quick: bool = False, show_plots: bool = True
) -> dict:
...
Examples:
  ...
  python run_bandit_experiments.py --all --quick
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Run specific SCM(s). Use space-separated list for multiple: --scm frontdoor four_variable six_variable",
    )
    parser.add_argument("--all", action="store_true", help="Run all SCMs")
    parser.add_argument(
        "--quick", action="store_true", help="Quick mode (T=1000, 50 trials)"
    )
    ...
    results = run_all_experiments(
        scm_names, quick=args.quick, show_plots=not args.no_plots
    )
