...
from typing import Set, List, Tuple, FrozenSet, AbstractSet, Dict, Any, Optional
import random
import itertools
...
class POMISMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'max_recursion_depth': max(self.recursion_depths) if self.recursion_depths else 0,
            'mean_recursion_depth': sum(self.recursion_depths) / len(self.recursion_depths) if self.recursion_depths else 0,
            'final_pomis_set_size': len(self.final_pomis_set) if self.final_pomis_set else 0,
            'final_pomis_set_hash': hash(frozenset(self.final_pomis_set)) if self.final_pomis_set else 0
        }
...
class InstrumentedPOMISComputer:
    ...
    def compute_pomis_with_order(self, G: CausalDiagram, Y: str, 
                                 custom_order: Optional[Tuple[str, ...]] = None) -> Set[FrozenSet[str]]:
        ...
        if custom_order is not None:
            ...
            G.causal_order = lambda backward=False: custom_order if backward else tuple(reversed(custom_order))
    ...
    def _instrumented_subpomis(self, G: CausalDiagram, Y: str, Ws: List[str], obs=None) -> Set[FrozenSet[str]]:
        ...
        try:
            ...
            for i, W_i in enumerate(Ws):
                ...
                if not (Xs & new_obs):
                    ...
                    if new_Ws:
                        result = self._instrumented_subpomis(G.do(Xs)[Ts | Xs], Y, new_Ws, new_obs)
                        out.extend(result)
                else:
...
def generate_topological_orders_strategies(G: CausalDiagram) -> Dict[str, Tuple[str, ...]]:
